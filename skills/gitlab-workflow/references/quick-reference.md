...
glab issue view 456 --comments     # View with comments
glab issue close 456               # Close issue
glab issue update 456 --label "in-progress" --unlabel "to-do"
glab issue note 456 -m "Comment"   # Add comment
```
...
glab ci run -V KEY=value           # Trigger with variable
...
glab repo create project-name      # Create repository
```
...
# Paginated (use query params in URL)
glab api --paginate "projects/:id/issues?per_page=100"
```
...
glab variable set VAR_NAME value   # Set variable
```
...
GITLAB_ISSUE_PROJECT=group/proj    # Default issue project
```
...
- `glab pipeline` — Pipeline operations
...
6. Enable shell completion: `glab completion --shell zsh`
